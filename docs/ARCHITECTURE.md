...
**Variable rename heuristic:** when a `updateWidget` edit removes one variable from a widget and adds another, the store treats it as a rename — batch row values follow the new name, and any other widgets referencing the old name are rewritten. The heuristic is set-diff over the widget's variables. The detection deliberately counts the empty placeholder `{{}}` as a (transient) variable (`detectVariables(..., { includeEmpty: true })`), so renaming by backspacing a name down to `{{}}` and retyping is safe: the edit passes through `{{old}} → {{}} → {{new}}` as a chain of single renames (`old → "" → new`) and the row value rides along under the empty key rather than orphaning. Remaining limitation: an edit that also destroys a brace (e.g. `{{name}}` → `{{names}` → `{{names}}`) passes through a state with no *complete* placeholder, which still reads as a pure removal then addition and orphans the value; select-and-replace and paste also work. Orphaned values reappear if the original name is typed back.
